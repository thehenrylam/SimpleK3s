...
| Resource          | Rate (Hourly)   | Rate (Monthly)  | Comment |
| ----------------: | :-------------- | :-------------- | :------ |
| t4g.small         |  $0.0168/hr     | $12.27/mo       |         |
| t4g.medium        |  $0.0336/hr     | $24.54/mo       |         |
| t4g.large         |  $0.0672/hr     | $49.09/mo       |         |
| t4g.xlarge        |  $0.1344/hr     | $98.18/mo       |         |
| m6g.medium        |  $0.039/hr      | $28.49/mo       |         |
| m6g.large         |  $0.077/hr      | $56.25/mo       |         |
| m6g.xlarge        |  $0.154/hr      | $112.50/mo      |         |
| c6g.medium        |  $0.034/hr      | $24.84/mo       |         |
| c6g.large         |  $0.068/hr      | $49.67/mo       |         |
| c6g.xlarge        |  $0.136/hr      | $99.35/mo       |         |
| db.r6g.2xlarge    |  $0.899/hr      | $656.72/mo      | for DB instances only |
| db.r6g.4xlarge    |  $1.798/hr      | $1,313.44/mo    | for DB instances only |
...
| **Total** (light)       | 7.5 GB/mo           | $0.30/mo          | Exclude ArgoCD and use Prometheus (light)               |
| **Total** (standard)    | 61.0 GB/mo          | $2.44/mo          | Exclude ArgoCD and use Prometheus (standard)            |
| **Total** (heavy)       | 1501.0 GB/mo        | $60.04/mo         | Exclude ArgoCD and use Prometheus (heavy)               |

- The control-plane PVC footprint (the `EBS — PVC (light)` line in Scenarios 0–2) is the monitoring stack: **Prometheus 8 GB + Grafana 0.75 GB + Alertmanager 0.75 GB ≈ 10 GB per node**. This is down from ~28 GB now that Thanos offloads long-term metrics to S3, leaving Prometheus only a short local window (default 6h) on the PVC. Note that Longhorn replicates this disk into every AZ the cluster runs in, so the EBS line below is the per-node disk multiplied by the node count.
...
| EC2 — Workers (System) | $73.63            | 3 × t4g.medium (on-demand) |
| EBS — Workers (System) | $3.84             | 3 × 16 GB gp3             |
...
The out-of-the-box configuration: 3 control-plane nodes plus 1 dedicated agent, running K3s. System pods (ArgoCD, Grafana, Prometheus, Traefik, etc.) are distributed across both planes — no taints are set, and the scheduler places the heaviest pods on the agent because it is the emptiest node (measured: Prometheus, the largest single consumer, lands there unprompted). Provisioning further agents is flexible — Karpenter is idle until workloads demand scale-out.

| Line Item                   | Est. Monthly Cost | Detail                        |
| --------------------------: | :---------------- | :---------------------------- |
| EC2 — Control Plane         | $73.63            | 3 × t4g.medium (on-demand)    |
| EBS — Control Plane         | $3.84             | 3 × 16 GB gp3                 |
| EC2 — Dedicated Agent       | $24.54            | 1 × t4g.medium (on-demand)    |
| EBS — Dedicated Agent       | $1.28             | 1 × 16 GB gp3                 |
| EBS — PVC (light)           | $6.72             | 3 × 28 GB gp3                 |
| Cross AZ — PVC (light)      | $0.30             | Referred to the Cross AZ usage table above |
| Network Load Balancer       | $16.43            | 1 NLB (low traffic)           |
| Public IPv4 — Nodes         | $14.60            | 4 addresses × $3.65/mo        |
| **Total**                   | **$141.35**       |                               |

> SimpleK3s is ***~23.5%*** cheaper than a comparable EKS setup without changing anything

---

### Scenario 2 — Cost Optimized (High Availability is Disabled)
...
| Line Item                   | Est. Monthly Cost | Detail                      |
| --------------------------: | :---------------- | :-------------------------- |
| EC2 — Control Plane         | $24.54            | 1 × t4g.medium (on-demand)  |
| EBS — Control Plane         | $1.28             | 1 × 16 GB gp3               |
| EC2 — Dedicated Agent       | $49.09            | 2 × t4g.medium (on-demand)  |
| EBS — Dedicated Agent       | $2.56             | 2 × 16 GB gp3               |
| EBS — PVC (light)           | $2.24             | 1 × 28 GB gp3               |
| Cross AZ — PVC (light)      | $0.00             | No multi AZ setup, so no cross AZ costs |
| Network Load Balancer       | $16.43            | 1 NLB (low traffic)         |
| Public IPv4 — Nodes         | $10.95            | 3 addresses × $3.65/mo      |
| **Total**                   | **$107.09**       |                             |

> Cost Optimized SimpleK3s is ***~42.0%*** cheaper than a comparable EKS setup (without HA)<br/>
> Cost Optimized SimpleK3s is ***~24.2%*** cheaper than a SimpleK3s default setup
...
- Assumed tech stack: (MERN-like stack, using Supabase instead of MongoDB)
    - Frontend: ReactJS 
...
| Line Item                  | Est. Monthly Cost | Detail                                               |
| -------------------------: | :---------------- | :--------------------------------------------------- |
| SimpleK3s Control Plane    | $107.09           | Pricing metrics of Scenario 2                        |
...
> ***Reminder***: This setup is cheaper than a SimpleK3s baseline setup (default settings)! (**$136.42/mo** vs **$141.35/mo**)
...
| Line Item                  | Est. Monthly Cost | Detail                                               |
| -------------------------: | :---------------- | :--------------------------------------------------- |
| SimpleK3s Control Plane    | $141.35           | Pricing metrics of Scenario 1                        |
| EC2 — Workers (On-Demand)  | $42.73            | 1.5 × m6g.medium (1.5 x $0.039/hr)                   |
| EBS — Workers              | $2.88             | 1.5 × 24 GB gp3                                      |
| EBS — PVC (standard)       | $15.36            | 3 × 64 GB gp3 (Full HA multi-AZ)                     |
| Cross AZ — PVC (standard)  | $2.44             | Referred to the Cross AZ usage table above           |
| Public IPv4 - Workers      | $5.48             | 1.5 × addresses                                      |
...
> ***Reminder***: This complete SimpleK3s setup is **~15.6%** cheaper than using a comparable EKS setup. Meaning if you have a 1 year runway to run your infrastructure, using SimpleK3s will lengthen your runway by ~2 ADDITIONAL months!
...
- Each concurrent user will on average, make 1 RPS (request per second)
- 1 × c6g.xlarge can handle 10K RPS (if m6g.medium w/ 1vCPU can handle 2.5K RPS, then c6g.xlarge w/ 4vCPU could handle 4x2.5K RPS)
    - From the assumptions we would need 100 × c6g.xlarge to accommodate 1M users
...
| Line Item                       | Est. Monthly Cost | Detail                                                        |
| ------------------------------: | :---------------- | :------------------------------------------------------------ |
| EC2 — Control Plane             | $294.54           | 3 × t4g.xlarge (high workload for ArgoCD + monitoring)        |
...
| 1 — Default Settings                | $141.35/mo    | $184.87/mo     | SimpleK3s ~23.5% cheaper |
| 2 — Cost Optimized                  | $107.09/mo    | $184.87/mo     | SimpleK3s ~42.0% cheaper |
| 3 — Minimal Workload (~1K users)    | $136.42/mo    | $214.20/mo     | SimpleK3s ~36.3% cheaper |
| 4 — Standard Workload (~2.5K users) | $235.24/mo    | $278.76/mo     | SimpleK3s ~15.6% cheaper |
...
> EKS equivalent on Scenario 5 makes the assumption that nothing is needed to be modified from the EKS setup from Scenario 0.<br/>
> However, it's very doubtful that it would be the case, because the compute requirements for ArgoCD/Grafana/Prometheus/Kyverno scale with the size of the cluster, and at this size (serving 1M concurrent users), the additional cost to provision for support pods on EKS would be significant, and its cost would be roughly the same as SimpleK3s. 

### When SimpleK3s Wins

- **Scenarios 1–4 (early-to-mid stage startups)** — SimpleK3s runs ~15–42% cheaper than an equivalent EKS setup. The savings come primarily from avoiding the $73/mo control plane flat fee and from having all tooling pre-bundled rather than self-installed on top of EKS.
...
- **Offload Data Storage from EBS to S3:** EBS is deceptively expensive and is trickier to scale than S3. Especially when using them as PVCs, which you'll need to pay extra for HA and cross AZ transfers.
  ...
  | Storage | Effective rate (per GB-mo) | Notes |
  |---|---|---|
  | EBS gp3 (single copy) | ~$0.08 | Base rate; billed even when idle. |
  | EBS gp3 via Longhorn PVC (3 replicas, 3-AZ) | ~$0.24 + cross-AZ transfer | Each replica is a full EBS volume; every write also crosses AZs at $0.02–$0.04/GB. |
  | S3 Standard | ~$0.023 | ~90% cheaper than replicated EBS, and scales without pre-provisioning. |

  This is exactly why **Prometheus metrics are stored on S3 via the built-in Thanos sidecar**. Prometheus keeps only a short local window (default 6h) on a small PVC (default `8Gi` instead of `20Gi`), while the Thanos sidecar continuously ships TSDB blocks to S3 and the Store Gateway/Querier serve historical queries from there. The Thanos Compactor enforces tiered S3 retention (raw 30d, 5m-downsampled 90d, 1h-downsampled 1y), so long-term storage stays cheap and bounded instead of growing as triple-replicated EBS. In total this drops the monitoring stack's per-node disk from ~28 GB to ~10 GB (Prometheus 8 + Grafana 0.75 + Alertmanager 0.75), reflected in the `EBS — PVC (light)` lines in Scenarios 0–2.
