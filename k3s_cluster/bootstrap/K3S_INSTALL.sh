...
wait_for_k3s_api() {
  ...
  for i in {1..18}; do
    if sudo kubectl get --raw=/readyz >/dev/null 2>&1; then
      log_okay "K3s API is ready!"
      return 0
    fi
    sleep 10
  done
  log_fail "K3s API is not ready after 180s"
  return 1
}
# Wait for a kubectl resource to be available
wait_for_resource() {
  log_info "Waiting for resource to be available..."
  # usage: wait_for_resource <kubectl args that must succeed>
  for i in {1..18}; do
    if eval "$@" >/dev/null 2>&1; then
      log_okay "Resource is available!"
      return 0
    fi
    sleep 10
  done
  log_fail "Resource is not available after 180s"
  return 1
}
...
function main_setup() {
  ...
  if [ $COUNT_INDEX -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
      ...
      log_fail "K3s installation command failed on PRIMARY node!"
      exit 1
    fi

    # Read the generated token (server token file)
    log_info "Store K3s token into AWS SSM Parameter Store..."
    TOKEN="$(sudo cat /var/lib/rancher/k3s/server/token)"

    if [[ -z "$TOKEN" || "$TOKEN" == "$PLACEHOLDER_TOKEN" ]]; then
      log_fail "token read from disk looks invalid"
      exit 1
    fi
    ...
    if [ $? -eq 0 ]; then
      log_okay "Stored K3s token into AWS SSM Parameter Store!"
    else
      log_fail "Something went wrong when storing K3s token into AWS SSM Parameter Store!"
    fi 
  fi
  ...
  if [ ! $COUNT_INDEX -eq 0 ]; then    
    ...
    TOKEN="$(wait_for_real_k3s_token 180 2)" || {
      log_fail "Timed out waiting for k3s token to be populated in SSM."
      exit 1
    }
