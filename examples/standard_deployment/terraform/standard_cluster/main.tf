...
#
# PROFILE: standard — 3 control-plane + 1 agent, all t4g.medium
#
# Copy over main.tf to use this profile.
#
# The reference deployment. Workloads are NOT pinned: no taints are set, and the
# scheduler places the heavy pods on the agent on its own, because the agent is
# the emptiest node. Measured on a full stack (Longhorn + monitoring + ArgoCD):
# Prometheus (the largest consumer, ~967Mi) landed on the agent unprompted.
#
# Why an agent node at all: an agent does not run the k3s server components, so
# it carries ~131m less CPU and ~788Mi less RAM of non-pod overhead than a
# control-plane node (measured: control plane 128-258m / 1584-1924Mi, agent
# 48m / 932Mi). On a 4 GiB node that is ~20% of the machine recovered simply by
# not running etcd and the apiserver.
#
# Measured steady state, full stack:
#   control planes  76-77% memory   (roughly half of which is the k3s server
#                                    itself, invisible to the scheduler)
#   agent           62% memory
#   Karpenter       provisions in ~97s, folds back to zero in ~7m
#
# HEADROOM WARNING: the control planes sit near their practical ceiling. With
# Longhorn's per-node DaemonSets the floor on a t4g.medium control plane is
# ~58% before anything schedulable lands. If you add subsystems or your own
# charts, move the control planes to t4g.large rather than shrinking elsewhere —
# the floor is the control plane itself, not the workloads.
...
module "k3s_cluster" {
  ...
  subsystems = {
    ...
    karpenter = {
      ...
      # Node size envelope (#131) — every Karpenter node lands between a
      # t4g.medium and a t4g.xlarge. The floor keeps nodes big enough to carry
      # the DaemonSet tax plus real work; the ceiling stops a large pod pulling
      # in a t4g.2xlarge unprompted (measured: two 1200m pods did exactly that).
      min_instance_cpu        = 2
      max_instance_cpu        = 4
      min_instance_memory_mib = 4096
      max_instance_memory_mib = 16384
      # Aggregate spend backstop. For t4g, price is exactly $0.0084/GiB-hr at
      # every size, so memory_limit is an exact cap: $/mo = GiB x $6.132.
      # 64Gi ~= $392/mo worst case. Lower it to tighten the ceiling.
      #
      # node_limit is intentionally omitted: capping node COUNT fights the point
      # of autoscaling, and cpu/memory already bound spend. Uncomment only if the
      # count itself is the constraint (e.g. subnet IP exhaustion).
      # node_limit      = "3"
      cpu_limit         = "16"
      memory_limit      = "64Gi"
