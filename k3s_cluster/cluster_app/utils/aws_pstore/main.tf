locals {
    ...
    # Pstore variables
    pstore_prefix   = "pstore-${var.module_name}-"

    # Reformat the list of pstore_data to be indexed by name instead
    pstore_data_to_create = { for p in var.pstore_data : p.name => p if p.create == true }
    pstore_data_to_attach = { for p in var.pstore_data : p.name => p if p.create != true }
...
resource "aws_ssm_parameter" "ssm_params_dynamic" {
    for_each    = local.pstore_data_to_create

    description = each.value.desc
    type        = each.value.encrypted ? "SecureString" : "String"
    name        = each.value.name
    value       = each.value.value == null ? var.uninitialized : each.value.value

    tags = merge(local.tags_default, {
        Name    = "${local.pstore_prefix}${each.value.name}"
    })
...
data "aws_ssm_parameter" "pstores" {
    for_each        = local.pstore_data_to_attach
    name            = each.key
    with_decryption = each.value.encrypted
}

# Output Data : Used to initialize the IAM policies, and for use outside of the module itself 
locals {
    # Centralize all of our pstore data to make it easier to set up other components easier
    processed_pstores_create = { for name, o in aws_ssm_parameter.ssm_params_dynamic : name => {
        alias       = local.pstore_data_to_create[name].alias
        name        = o.name
        encrypted   = local.pstore_data_to_create[name].encrypted
        region      = o.region
        ipolicy_arn = "${local.ipolicy_pstore_arn_root}/${o.name}"
    } }

    processed_pstores_attach = { for name, o in data.aws_ssm_parameter.pstores : name => {
        alias       = local.pstore_data_to_attach[name].alias
        name        = o.name
        encrypted   = local.pstore_data_to_attach[name].encrypted
        region      = o.region
        ipolicy_arn = "${local.ipolicy_pstore_arn_root}/${o.name}"
    } }

    processed_pstores_merged = merge(local.processed_pstores_create, local.processed_pstores_attach)

    processed_pstores = { for p in var.pstore_data : p.name => local.processed_pstores_merged[p.name] }
}
